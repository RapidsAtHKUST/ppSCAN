...
#ifndef PPSCAN_GRAPHGALLOPINGSEARCHEXP_H
#define PPSCAN_GRAPHGALLOPINGSEARCHEXP_H
...
private:
    ...
    ui BinarySearchForGallopingSearch(vector<int> &array, ui offset_beg, ui offset_end, int val);
...
public:
    explicit GraphGallopingSearchExp(const char *dir_string, const char *eps_s, int min_u);

    void pSCAN();

    void Output(const char *eps_s, const char *miu);

    virtual ~GraphGallopingSearchExp();
};


#endif //PPSCAN_GRAPHGALLOPINGSEARCHEXP_H
