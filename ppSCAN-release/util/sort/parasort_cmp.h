...
static void _sort(size_t sz, T *list, Cmp functor) {
    std::sort(list, list + sz, functor);
}
...
template<typename T, typename Cmp>
static void _count(size_t sz, const T *list,
                   unsigned np, const T *range, size_t *bucket, Cmp functor) {
    for (unsigned n = 0; n < np; n++) bucket[n] = 0;
    for (size_t i = 0; i < sz; i++) {
        unsigned n;
        for (n = 0; n < np - 1; n++) {
            if (functor(list[i], range[n])) {
                bucket[n]++;
                break;
            }
        }
...
template<typename T, typename Cmp>
static void _reorder(size_t sz, const T *list,
                     unsigned np, const T *range, size_t *map, T *out, Cmp functor) {
    for (size_t i = 0; i < sz; i++) {
        unsigned n;
        for (n = 0; n < np - 1; n++) {
            if (functor(list[i], range[n])) {
...
template<typename T, typename Cmp>
void parasort(size_t sz, T *list, Cmp functor, unsigned np = 2, unsigned sf = 100) {
...
// We use these elements to split the array into even partitions.
    ...
    _sort(nSample, &sample[0], functor);
...
// This is done in parallel
    ...
    for (unsigned i = 0; i < np; i++) {
        ...
        threads[i] = std::thread(_count<T, Cmp>, length, &list[start], np, &range[0], &bucket[i * np], functor);
    }
...
// Step 4: Map the input array into np separate partitions.    
    ...
    for (unsigned i = 0; i < np; i++) {
        size_t start = i * part;
        size_t length = (i + 1 == np) ? sz - i * part : part;
        threads[i] = std::thread(_reorder<T, Cmp>, length, &list[start], np, &range[0], &tmp[i * np], &sorted[0],
                                 functor);
    }
    for (auto &&thread : threads) thread.join();
