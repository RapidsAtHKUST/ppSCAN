...
GraphExp0::GraphExp0(const char *_dir) {
    dir = string(_dir);
...
ui GraphExp0::binary_search(const int *array, ui b, ui e, int val) {
#ifdef _DEBUG_
...
void GraphExp0::cluster_noncore_vertices(int eps_a2, int eps_b2, int mu) {
...
void GraphExp0::output(const char *eps_s, const char *miu) {
...
void GraphExp0::pSCAN(const char *eps_s, int _miu) {
    get_eps(eps_s);
    miu = _miu;
    ...
    while (true) {
        ...
        // 1st: Check Core
        for (ui idx = pstart[u]; idx < pstart[u + 1] && similar_degree[u] < miu && effective_degree[u] >= miu; idx++) {
            if (min_cn[idx] >= 0) {
                int v = edges[idx];
        ...
        // 2nd: Cluster Core-Core, correctness guaranteed by the fact that the latter checked one will always check the previous checked one
        // However, (u,v) and (v,u) may conduct two joins, thus three invocations of find_root(u) and find_root(v)
        if (similar_degree[u] >= miu) {
            for (auto idx = pstart[u]; idx < pstart[u + 1]; idx++) {
                auto v = edges[idx];
                if (similar_degree[v] >= miu && find_root(u) != find_root(v)) {
                    if (min_cn[idx] >= 0) {
                        min_cn[idx] = min_cn[reverse[idx]] = similar_check_OP(u, idx, eps_a2, eps_b2);
                    }
...
int GraphExp0::check_common_neighbor(int u, int v, int c) {
    int cn = 2;
...
int GraphExp0::similar_check_OP(int u, ui idx, int eps_a2, int eps_b2) {
    int v = edges[idx];
...
int GraphExp0::compute_common_neighbor_lowerbound(int du, int dv, int eps_a2, int eps_b2) {
...
void GraphExp0::prune_and_cross_link(int eps_a2, int eps_b2, int miu, int *cores, int &cores_e) {
...
void GraphExp0::my_union(int u, int v) {
    int ru = find_root(u);
    int rv = find_root(v);
...
void GraphExp0::get_eps(const char *eps_s) {
